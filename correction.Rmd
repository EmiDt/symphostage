...
library(magrittr)
#<<<<<<< HEAD
path <- "F:\\Stage Ecofog\\symphostage\\" # path to all data files currently in the same place
#setwd(path)
...
DW_out <- read.csv(file.path(path, "PoidsSecOutliers.csv"),header = T, dec = ",", sep=";")
...
Leaf_Area_data_exclude <- LA_data_Leaf_ordered %>% dplyr::select(-Area) %>% full_join(LA_data_exclude_final2) %>% 
mutate(feuille_f  = ifelse(is.na(disordered), n_feuille,disordered)) %>% 
  select( n_parcelle, n_carre, n_arbre , feuille_f, Area) %>% 
  rename( n_feuille = feuille_f)
...
data_full2 %>% 
  mutate(ID = as.factor(paste(n_parcelle, n_carre, n_arbre, sep="_"))) %>%  dplyr::select(ID,n_feuille,Area_exclude) %>%
  filter(ID == "11_4_983" | ID == "11_1_742")
```
