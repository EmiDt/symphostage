...
```{r}
G_neighbour <- read.csv("C:\\Users\\emduc\\Desktop\\Drive\\symphostage\\Full_Gneighbour.csv",header = T, dec = ".", sep=",")
...
ACP_data2<- environment_trait2 %>% 
  ...
  select( dem,aspect, TRI, morphotype_field) 
  


res.pca2 <- PCA(ACP_data2, quali.sup = 4, graph = F)
...
In my opinion TRI is a better descriptor for abiotic variables because microheterogeneity in soil slope and elevation could have an impact at the individual scale. Also slope is integrated in wetness index and once again it would biologically easier to interpret than just more or less steep area..
...
Then aspect variable and TRI are in every projections almost orthogonal so aspect give an information that cannot be resume by TRI 
...
ACP_data5<- environment_trait2 %>% 
  select( wetness, d_creek, TRI, dem, aspect) 

res.pca5 <- PCA(ACP_data5, graph = F)
...
ACP_data3<- environment_trait2 %>% 
  select(wetness, dem,d_creek, aspect, TRI,  diameter, LMDC, SLA, Chloro_content, LT_mean, morphotype, Competition) 
