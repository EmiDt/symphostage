...
Trait_mean_individuals_vect<- Traits_brut_data %>% 
  select(n_parcelle, n_carre, n_arbre, morphotype, Dawkins, diameter, Bois, Xutm, Yutm) %>% 
...
ACP_data<- environment_trait %>% 
  select(d_log_gap, d_creek, waterlog, wetness, dem, slope,curvature,aspect, TRI, wtd, hydromorphy,drainages) 


res.pca <- PCA(ACP_data, quali.sup = c(10,11,12))
plot.PCA(res.pca, axes=c(1,2), choix="var" , habillage= "wtd")
```
...
ACP_data<- environment_trait %>% 
  select(waterlog, wetness, dem, slope, wtd, hydromorphy,drainages, LMDC, SLA, Chloro_content, LT_mean, morphotype, diameter, TRI) 


res.pca <- PCA(ACP_data, quali.sup = c(6,5,7,12))
plot.PCA(res.pca, axes=c(1,2), choix="var" , habillage= "morphotype")
```
