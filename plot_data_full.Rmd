...
Trait_mean_individuals_vect<- Traits_brut_data %>% 
  select(n_parcelle, n_carre, n_arbre, morphotype, Dawkins, diameter, Xutm, Yutm, wood_presence) %>% 
  group_by(n_parcelle, n_carre, n_arbre, morphotype, Dawkins, diameter, wood_presence) %>%
    summarize()
```
...
Trait_mean_transform_final <- merge(Trait_mean_transform, Trait_mean_individuals_vect, by = c("n_parcelle", "n_carre", "n_arbre") ) %>%
  ...
  filter( SLA < 200) %>% 
  filter(Dawkins != "") %>% 
  mutate(wood_presence = as.factor(as.character(wood_presence))) %>% 
...
# environment data
```{r}
environment <- read.csv("C:\\Users\\emduc\\Desktop\\Drive\\symphostage\\environment\\Paracou\\environmenttest.csv",header = T, dec = ",", sep=";") %>% 
...
ACP_data<- Trait_mean_transform_final %>% 
  select(LMDC, SLA, Chloro_content, LT_mean, morphotype, diameter) 


res.pca <- PCA(ACP_data, quali.sup = 5, graph= F)
...
ACP_data2<- environment_trait %>% 
  mutate(hydromorphy = as.factor(hydromorphy)) %>% 
  filter(!is.na(hydromorphy)) %>% 
  select(d_log_gap, d_creek, waterlog, wetness, dem, slope,curvature,aspect, TRI, morphotype) 
  


res.pca2 <- PCA(ACP_data2, quali.sup = 10, graph = F)

ind2 <- fviz_pca_ind(res.pca2, label="none", habillage = ACP_data2$morphotype,
             addEllipses=TRUE, ellipse.level=0.95)
 
var2 <-  fviz_pca_var(res.pca2, alpha.var="contrib")

both2 <- fviz_pca_biplot(res.pca2, label ="var")

plot_grid(ind2, var2, both2  , labels=c("A", "B","C"), ncol = 2, nrow = 2)
```
...
ACP_data3<- environment_trait %>% 
  select(waterlog, wetness, dem, slope, TRI,  diameter, LMDC, SLA, Chloro_content, LT_mean, morphotype) 


res.pca3 <- PCA(ACP_data3, quali.sup = 11, graph= F)

 ind3 <- fviz_pca_ind(res.pca3, label="none", habillage=ACP_data3$morphotype,
             addEllipses=TRUE, ellipse.level=0.95)
var3 <-  fviz_pca_var(res.pca3, alpha.var="contrib")
 both3 <- fviz_pca_biplot(res.pca3, label ="var")
 
 plot_grid(ind3, var3, both3  , labels=c("A", "B","C"), ncol = 2, nrow = 2)
```
