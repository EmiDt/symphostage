...
symphonia_stage <- read.csv(file.path(path, "data_merging\\Full_traits_data.csv"),header = T, dec = ",", sep=";")

symphonia_stage <- symphonia_stage %>%  group_by(n_parcelle, n_carre, n_arbre) %>% 
 dplyr::summarize() %>% 
...
Paracoutest <- paracoutree %>% 
  ...
  dplyr::rename( PointType = species)

rownames(Paracoutest) <- Paracoutest$PointName
Paracoutest <- dbmss::wmppp(Paracoutest, window = owin(c(0,250), c(0,250), unitname=c("metres", "metres")))
```
owin(c(0,115), c(4,109)
...
```
  ...
  # EstSymphonia[grep("Symphonia_", Paracoutest$marks$PointType)] <- TRUE #remplacement par TRUE des place de dans le vecteur qui corresponde à celle dans lesquelles il y a le pattern Symphonia_
...
```{r}
Hegyi  <- apply(Voisins, 2, function(EstVoisin) sum(Paracoutest[EstVoisin]$marks$PointWeight/Distances[EstVoisin]))
```
