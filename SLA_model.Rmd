...
step_data <- soil_data %>%  dplyr::select(-X, -n_parcelle, -n_carre, -n_arbre, -LT_mean, -LMDC, -Area_exclude, -Chloro_content) %>% filter(!is.na(Dawkins))
...
```{r eval=FALSE, include=FALSE}
summary(lm(formula = SLA ~ wetness + lBAL + d_creek + diameter + dem + 
    alt_creek, data = step_data))
```
