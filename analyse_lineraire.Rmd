...
```{r}
SLA_model<- lm(formula = SLA ~  aspect + diameter+ lBAL:diameter + d_creek:diameter  , data = data_standar[,4:9 &14]) 
...
```{r}
rbind(data_standar[76,], data_standar[151,],data_standar[363,], data_standar[175,])
```
...
LT_model <- lm(LT_mean~ wetness + lBAL + d_creek  + diameter + aspect + aspect : diameter+
 d_gap + lBAL:diameter+ d_gap:diameter + diameter:wetness + diameter:d_creek , data=data_standar[,4:11])
...
LT_model <- lm(LT_mean~ wetness + lBAL + d_creek  + diameter +
 diameter:lBAL , data=data_standar[, 4:11])
...
```{r LDMC model 1}
LDMC_model <- lm(LMDC~ wetness + d_creek + diameter + d_gap + lBAL + aspect+  diameter:lBAL + diameter:d_gap + wetness:diameter + d_creek:diameter, data = data_standar[, 4:9 & 13])
...
```
 Coefficient : 
  ...
  - diamter, positif coefficient LMDC increased when diameter increased which mean older tree seems to have more dense leaf tissues. It correspond to theecological strategy shift between under canopy layer tree ( young tree) and canopy or over canopy layer tree (aldult or post opening stage tree)
  - d_gap, positive coefficient LMDC increase when distance to the gap increase too.  That is not an intuitive result ...
  ...
  -diameter:d_creek, negative coefficient so diameter effect on lDMC decresed when d_creek decreased too. As diameter have a positive effect (LMDC increase with diameter) it means that for a tree with a given diameter far from the creek LMDC value will be smaller than a tree with the same diamter close to the creek. Its corroborates wetness negativ coefficient and soil watter avalaibility influence LMDC value. 
...
varcomp_data <- varcomp_data %>% 
  mutate(SLA = sqrt(SLA), Chloro_content= sqrt(Chloro_content), Area_exclude=log10(Area_exclude))
```
...
varcomp_resu <- as.data.frame(cbind(varcomp.Area, varcomp.LDMC, varcomp.LT, varcomp.SLA))

  varcomp_resu <- varcomp_resu %>% mutate( niveau = rownames(varcomp_resu)) %>% 
    dplyr::rename(Area = "varcomp.Area", LT="varcomp.LT", LDMC = "varcomp.LDMC", SLA = "varcomp.SLA")
...
```{r}
ggplot(varcomp_resu,aes(x = variable, y = value , fill = niveau ))+
