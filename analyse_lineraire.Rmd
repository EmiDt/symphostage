...
```{r}
SLA_model<- lm(formula = SLA ~  aspect + diameter+ lBAL:diameter + d_creek:diameter  , data = data_standar3[,4:10]) 
...
So SLA value can be explain by : 
  ...
  - diameter:creek, interaction has a positive coefficient so the effect of diameter increased on the SLA when the distance to the creek increase too. So for a given diameter, tree located far from the creek have smaller SLA value than tree with same diameter closer to the creek.
...
```{r}
 data_standar2<- data_standar %>%
  dplyr::select(-SLA, -LT_mean, -Chloro_content, -Area_exclude) 
  
# LMDC_model <- lm(LMDC~., data=data_standar2[,4:10])
#  step(object = LMDC_model, direction = "backward", k= log(n))

lm(formula = LMDC ~ wetness + diameter, data = data_standar2[, 
    4:10]) %>% summary
```


```{r echo=FALSE}
# LMDC_model <- lm(LMDC~wetness * lBAL * d_creek  * aspect *
#      d_gap* diameter, data=data_standar2[,4:10])
#  step(object = LMDC_model, direction = "backward", k= log(n))

lm(formula = LMDC ~ wetness + lBAL + d_creek + diameter + wetness:d_creek + 
    lBAL:diameter + d_creek:diameter, data = data_standar2[, 
    4:10])%>% summary()
```
...
LMDC and SLA influence the whole plant ecological strategy. Hight LMDC correspond to conservative species wich 
...
LT_model <- lm(LT_mean~ wetness + lBAL + d_creek  + diameter + aspect + aspect : diameter+
 d_gap + lBAL:diameter+ d_gap:diameter  , data=data_standar1[,4:10])
...
LT_model <- lm(LT_mean~ wetness + lBAL + d_creek  + diameter +
 diameter:lBAL , data=data_standar1[,4:10])
...
#  Chloro_model <- lm(Chloro_content~., data=data_standar4)
# step(object = Chloro_model, direction = "backward", k= log(n))
 
lm(formula = Chloro_content ~ wetness + diameter, data = data_standar4) %>%  summary
```
Once again only wetness and competition are selected, Adjusted R-squared:  0.09824

```{r echo =FALSE}
#  Chloro_model <- lm(Chloro_content~wetness * lBAL * d_creek  * diameter * 
#  d_gap, data=data_standar4)
# step(object = Chloro_model, direction = "backward", k= log(n))

lm(formula = Chloro_content ~ wetness + lBAL + d_creek + diameter + 
    lBAL:diameter + d_creek:diameter, data = data_standar4) %>% summary
```

d_creek, lBAL and their interaction with diameter have beeen added which raise the Adjusted R-squared to  0.1403.
