...
D_gap is a descriptor or the distance between the focal tree and the nearest light gap (here gap area area > 4m²), it is calculated with 1meter precision dcm.
...
Dcm is the digital canopy model it gives the mean height in a given point here our focal tree. It is not always reliable so we need to check if height value are cohorent with DBH data because there is a knonw allometry between height and DBH due to biological constrains like the "limite de flambage" in french. 

```{r}
dcm <- read.csv(file.path(path, "sympho_dcm.csv"),header = T, dec = ".", sep=",")%>% 
  dplyr::select(-X)

environment_trait <- dcm%>% 
  inner_join(environment_trait, by = c("n_parcelle", "n_carre", "n_arbre"))
...
## adding habitat topo category

```{r}
topo <-read.csv(file.path(path, "sympho_topo.csv"),header = T, dec = ".", sep=",") %>%   dplyr::select(-X)

environment_trait <- topo %>% 
  inner_join(environment_trait, by = c("n_parcelle", "n_carre", "n_arbre"))

```

# adding hillshade and new aspect

```{r}
hillshade <-read.csv(file.path(path, "sympho_ombre.csv"),header = T, dec = ",", sep=";")

environment_trait <- hillshade  %>% 
  inner_join(environment_trait, by = c("n_parcelle", "n_carre", "n_arbre"))
```
