...
#choose.files() # Use choose file to define the path above
```

```{r data upload}
DW <- read.csv(file.path(path, "PoidsSecSympho.csv"),header = T, dec = ",", sep=";") 
Ecorce<- read.csv(file.path(path, "Measures - Individuals2.csv"), header = T, dec=",", sep=";")
LTh <- read.csv(file.path(path, "Measures - Fresh.csv"), header = T, dec=".", sep=";")
LA <- read.csv(file.path(path, "Results Total Threshold- corrigé.csv"), header = T, dec=".", sep=";")
densite <- read.csv(file.path(path, "densite sympho.csv"),header = T, dec = ",", sep=";")
SPAD <- read.csv(file.path(path, "Measures - SPAD.csv"),header = T, dec = ".", sep=",")
...
LTh %>%  
  select(Parcelle, Carré, Numéro, Poids.frais, Feuille) %>% 
  mutate(Index= paste(Parcelle,Carré , Numéro, Feuille, sep="_"))
dw2 %>% as.tbl

LTh %<>% rename(Carre = Carré) %>%  rename(Numero = Numéro)
...
meme nombre d'observation dans les deux dataframe 
...
<!-- You can add a true caption to your figure with ```r, hypervolume, fig.cap="caption"``` -->

### LMDC~SLA

```{r reg LMDC LA, eval=F}
