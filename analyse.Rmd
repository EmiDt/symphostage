...
```{r , include=FALSE}
environment_standar2 <- environment_trait %>% 
  dplyr::select(n_parcelle, n_carre, n_arbre, Dawkins, Carbon , Phosphorus ,wetness,dem,slope, d_gap, alt_creek, TRI,hegyi) %>% 
  mutate(alt_creek = as.numeric((alt_creek)), Dawkins = as.factor(Dawkins),d_gap = log(d_gap))  

environment_standar2[,7:13] <- as.data.frame(scale(as.matrix(environment_standar2[,7:13]),scale = TRUE, center = TRUE), na.rm = TRUE) 

```

 
```{r echo=FALSE}
acp_envi <- PCA(environment_standar2[,7:13], graph =  FALSE)
fviz_pca_var (acp_envi , axes = c(1, 2), scaling =2, col.var = "contrib")
#fviz_screeplot(acp_envi, ncp=10)
```
```{r}
rm(environment_standar2)
```
...
```{r echo=FALSE}
envi_soil <- environment_standar %>%  filter(!is.na(Carbon)) %>% rename( alt_crique = alt_creek, Carbone = Carbon, d_trouée = d_gap , Phosphore = Phosphorus, Pente = Slope , Altitude = Alt)
...
```{r echo=FALSE}
pairs(envi_soil[,5:12], lower.panel = panel.smooth, upper.panel = panel.cor)
```
...
```{r}
summary(lm(bois$BT~bois$BD))
summary(lm(bois$BT~bois$WD))
```
...
covariation des trait with Pearson coefficient correlation 
```{r}
 cor(trait_60[,2:6],use = "na.or.complete",  method = "pearson")
cor(trait_Pl[,2:6],use = "na.or.complete",  method = "pearson")
 cor(trait_100[,2:6],use = "na.or.complete",  method = "pearson")
cor(trait_P[,2:6],use = "na.or.complete",  method = "pearson")
cor(trait_standar[,5:9],use = "na.or.complete",  method = "pearson")
```

LA and LT correlation  in global sample is similar to wtd 60-100 (0.11 and 0.03) whereas other cotegories have hight negative correlation coefficient (-0.57 ;-0.37 :-0.22)




## sma : strengh of correlation
```{r include=FALSE}
library(smatr)
trait_standar <- trait_standar %>%  left_join(environment_trait[,c("n_parcelle","n_carre", "n_arbre", "wtd")], by = c("n_parcelle", "n_carre", "n_arbre"))
```

```{r}
sma(SLA~LDMC, data= (trait_standar %>%   filter(wtd =="60-100")))
```
...
```{r}
par(mfrow=c(2,2))
plot(sma(SLA~LDMC, data= trait_standar), col = "black",lwd=3)
abline(sma(SLA~LDMC, data= (trait_standar %>%   filter(wtd =="Pente"))), col = "orange",lwd=3) ; abline( sma(SLA~LDMC, data= (trait_standar %>%   filter(wtd =="60-100"))), col="blue",lwd=3) ; abline(sma(SLA~LDMC, data= (trait_standar %>%   filter(wtd =="Plateau"))), col="red",lwd=3) ; abline( sma(SLA~LDMC, data= (trait_standar %>%   filter(wtd =="0-60"))),col="violet",lwd=3)

plot(sma(LA~LT, data= trait_standar), col = "black",lwd=3, main = " data in topographic/wattertable categories, 393 observations", sub = "black : total ;orange : slope; red : hilltop ; violet : bottomland 0-60; blue : bottomland 60-100")
abline(sma(LA~LT, data= (trait_standar %>%   filter(wtd =="Pente"))), col = "orange",lwd=3) ; abline( sma(LA~LT, data= (trait_standar %>%   filter(wtd =="60-100"))), col="blue",lwd=3) ; abline(sma(LA~LT, data= (trait_standar %>%   filter(wtd =="Plateau"))), col="red",lwd=3) ; abline( sma(LA~LT, data= (trait_standar %>%   filter(wtd =="0-60"))),col="violet",lwd=3)

plot(sma(LA~CC, data= trait_standar), col = "black",lwd=3)
abline(sma(LA~CC, data= (trait_standar %>%   filter(wtd =="Pente"))), col = "orange",lwd=3) ; abline( sma(LA~CC, data= (trait_standar %>%   filter(wtd =="60-100"))), col="blue",lwd=3) ; abline(sma(LA~CC, data= (trait_standar %>%   filter(wtd =="Plateau"))), col="red",lwd=3) ; abline( sma(LA~CC, data= (trait_standar %>%   filter(wtd =="0-60"))),col="violet",lwd=3)

plot(sma(LA~SLA, data= trait_standar), col = "black",lwd=3)
abline(sma(LA~SLA, data= (trait_standar %>%   filter(wtd =="Pente"))), col = "orange",lwd=3) ; abline( sma(LA~SLA, data= (trait_standar %>%   filter(wtd =="60-100"))), col="blue",lwd=3) ; abline(sma(LA~SLA, data= (trait_standar %>%   filter(wtd =="Plateau"))), col="red",lwd=3) ; abline( sma(LA~SLA, data= (trait_standar %>%   filter(wtd =="0-60"))),col="violet",lwd=3)
#rm("trait_100", "trait_P", "trait_Pl", "trait_60", "BF_100", "P", "Pl", "BF_60")

plot(sma(LA~LT, data= trait_standar %>%   filter(wtd =="60-100")), col = "black",lwd=3, main = " data in bottomland 60-100 categories, 34 observations")

plot(sma(LA~LT, data= trait_standar ), col = "black",lwd=3, main = " data total, 402 observations")
```
control if covariation between LA and LT are modified in stable data for LT , it seems taht only global sample if modified and now follow 0-60 and hilltop variaiton pattern. 
```{r}
LT_decrease <- c("3","7","2","16","8","9","10","11","12","6") 
data_LT <- trait_standar %>% mutate(n_parcelle = as.factor(n_parcelle)) %>%  filter (!n_parcelle %in% LT_decrease )

plot(sma(LA~LT, data= data_LT), col = "black",lwd=3, main = " LT stable, 183 observations", sub = "black : total ;orange : slope; red : hilltop ; violet : bottomland 0-60; blue : bottomland 60-100")
abline(sma(LA~LT, data= (data_LT %>%   filter(wtd =="Pente"))), col = "orange",lwd=3) ; abline( sma(LA~LT, data= (data_LT %>%   filter(wtd =="60-100"))), col="blue",lwd=3) ; abline(sma(LA~LT, data= (data_LT %>%   filter(wtd =="Plateau"))), col="red",lwd=3) ; abline( sma(LA~LT, data= (data_LT %>%   filter(wtd =="0-60"))),col="violet",lwd=3)


```

control if in stable data for LT covariation are modified or not and they are not 

```{r}
trait_LT <- PCA(data_LT[,5:9], graph = F)
fviz_pca_var(trait_LT , axes = c(1, 2), label ="var", scaling =2, col.var="contrib", col.ind="grey", title = " LT stable PCA")
```

```{r}
 cor(data_LT[,5:10] %>% filter(wtd =="Pente") %>% select(-wtd),use = "na.or.complete",  method = "pearson")
cor(data_LT[,5:10] %>% filter(wtd =="Plateau")%>% select(-wtd),use = "na.or.complete",  method = "pearson")
 cor(data_LT[,5:10] %>% filter(wtd =="0-60")%>% select(-wtd),use = "na.or.complete",  method = "pearson")
cor(data_LT[,5:10] %>% filter(wtd =="60-100")%>% select(-wtd),use = "na.or.complete",  method = "pearson")
cor(data_LT[,5:9],use = "na.or.complete",  method = "pearson")
```


```{r}
 obj <- sma(SLA~LDMC, data= trait_standar)
intercept <- obj$coef[[1]][1, 1]
slope<- obj$coef[[1]][2, 1]

ggplot(trait_standar, aes(x=SLA, y=LDMC))+
geom_point()+
  geom_abline(data=trait_standar, aes(slope=slope, intercept=intercept))+
   annotate("text", x = 2, y=2.3, label = "SMA R-squared:  0.35 
            slope : -0.99")
...
test de variance partionning sur les donnée de LT non décroissante

```{r include=FALSE}
LT_decrease <- c("3","7","2","16","8","9","10","11","12","6") 
varcomp_LT <- varcomp_data %>% mutate(n_parcelle = as.factor(n_parcelle)) %>%  filter (!n_parcelle %in% LT_decrease ) 

varcomp.LT_stable<-varcomp(lme(LT~1,
random=~1|n_parcelle/morphotype_field/Dawkins/n_arbre, data = varcomp_LT ,na.action = na.omit),1)
...
fviz_pca_biplot (acp_envi , axes = c(1, 2), scaling =1, habillage = 4,  col.var = "black", label = "var")
```
...
```{r}
ggplot(environment_trait,aes(wetness, col = morphotype_field, fill = morphotype_field)) +
...
```{r}
ggplot(environment_trait, aes(morphotype_field, LDMC))+
  geom_boxplot()
```

```{r}
ggplot(trait_standar, aes(LA, LT, color = morphotype_field))+
  geom_point()
```
...
#test <- hypervolume(trait_standar[,1:5],method='box')
#plot(test)

#varimp = hypervolume_variable_importance(test,verbose=FALSE)
#barplot(varimp) # contribution respective de variable au volume
```
